...
namespace GrammatikLaxen.Controllers
{
    public class SpelController : Controller
    {
        ...
        public ActionResult Spelet()
        {
            // Används till topplistan
            List<toppresultat> allTopList = db.GetTop10List();
            ViewBag.Topp = allTopList;
            ...
            return View(grammatikFraga);
        }

        //Används inte?
        [HttpPost]
        public ActionResult SpelKontroll(FragaSvar fra)
        {
            // objektet följer inte med (error)
            if (fra == null)
            {
                return RedirectToAction("Index");
            }
            // rätt svar
            //else if (svar == fra.Svar1 || svar == fra.Svar2 || svar == fra.Svar3 || svar == fra.Svar4 || svar == fra.Svar5 || svar == fra.Svar6 || svar == fra.Svar7 || svar == fra.Svar8)
            //{
            //    return RedirectToAction("Index");
            //}
            // fel svar (string svar)
            else
            {
                return RedirectToAction("ToppLista");
            }
        }

        public ActionResult LaddaSpelet()
        {
            // Används till topplistan
            List<toppresultat> allTopList = db.GetTop10List();
            ViewBag.Topp = allTopList;
            ...
            return PartialView("_Spelet", grammatikFraga);
        }

        public ActionResult SlutPåSpelet()
        {
